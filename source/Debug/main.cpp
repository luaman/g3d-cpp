...
{
public:
    // Takes an array of vertices, and an array of triangle indices 
    // to traverse them.
...
protected:
    ...
    Array< Table< int, float > > angleInner; // cosine of angle at [vertex][face]
    Array< Table< int, float > > angleLeft;  // cosine of angle at [adjacent vertex][face]
    Array< Table< int, float > > angleRight; // cosine of angle at [other adjacent vertex][face]

    Array< Table< int, double > > cota;  // cotan of agles opposite each segment [i][j] 
    Array< Table< int, double > > cotb;
    Array< double > AMixed;  // Area of "Voronoi" region surrounding each vertex [i] 
...
void Curvatures::initAMixed()
{
    ...
    int v, left, right;
    float lenLeft, lenRight; // length of opposite sides
    for(int i = 0; i < geometry.vertexArray.size(); ++i) {  // Init Amixed
        AMixed.append(0);
    }
    ...
    for(int i = 0; i < geometry.vertexArray.size(); ++i) {  // For each vertex
        for(int j = 0; j < adjacentFaces[i].size(); ++j) // For each face
        {
            ...
            //find vertex i
            if(face.vertexIndex[0] == i)      v = 0;
            else if(face.vertexIndex[1] == i) v = 1;
            else                              v = 2;
            left  = (v+1)%3;
            right = (v+2)%3;
            ...
            if(fabs(angleInner[i][faceIdx]) > 1.5708){  // Angle at i is obtuse
                AMixed[i] += tri.area()/2.0;
            } else if((fabs(angleLeft[ face.vertexIndex[left]][faceIdx]) > 1.5708) || 
                      (fabs(angleRight[face.vertexIndex[right]][faceIdx]) > 1.5708))
            {   // The triangle is obtuse
                AMixed[i] += tri.area()/4.0;
            } else { // tri is not obtuse 
                lenLeft = (tri.vertex(0)-tri.vertex(1)).squaredLength();
                lenRight = (tri.vertex(0)-tri.vertex(2)).squaredLength();
                AMixed[i] +=  0.125*( lenLeft*1/tan(angleLeft[face.     // Voronoi region
                                               vertexIndex[left]][faceIdx]) +   
                                      lenRight*1/tan(angleRight[face.
                                               vertexIndex[right]][faceIdx]));
            }
        }
    }
}
...
void Curvatures::gaussianCurvature(Array< double >& curvatures ) const
{
    ...
    for(int i = 0; i < geometry.vertexArray.size(); ++i) {  // For each vertex
         ...
         curvatures.append( (6.2831853071795862 - theta) / AMixed[i] );  // TODO: make 2Pi
    }
}
...
void Curvatures::meanCurvature(Array< double >& curvatures ) const
{
    ...
    Set< int > ringSet;
    Array< int > ring;
    Vector3 K; // Mean Curvature Normal
    ...
    for(int i = 0; i < geometry.vertexArray.size(); ++i) {  // For each vertex
         ...
         for(int j = 0; j < adjacentFaces[i].size(); ++j) // For each face
         {
             ringSet.insert(faces[adjacentFaces[i][j]].vertexIndex[0]);  //add all verts
             ringSet.insert(faces[adjacentFaces[i][j]].vertexIndex[1]);
             ringSet.insert(faces[adjacentFaces[i][j]].vertexIndex[2]);
         }

         ringSet.remove(i); // ringSet contains N1 ring only
...
{
    Array< double > kh, kg;  // mean and gaussian curvature
    ...
    for(int i = 0; i < geometry.vertexArray.size(); ++i) {  // For each vertex
       dxi = kh[i]*kh[i] - kg[i];
       if(dxi < 0) dxi = 0;  // threshold at 0 
       
       k1.append(kh[i] + sqrt(dxi));
       k2.append(kh[i] - sqrt(dxi));
    }
}
...
void Curvatures::principalDirections( Array< Vector3 >& T1, Array< Vector3 >& T2) const
{
    ...
    Set< int > ringSet;
    Array< int > ring;        // 1-ring neighbors of vi
    Vector3 vi, vj, dvji;     // vertex, neighbor, and vj-vi
    Vector3 W;                // Setup for Householder matrix
    Matrix3 Q = Matrix3::ZERO;// Householder matrix
    Matrix3 M = Matrix3::ZERO;
    Matrix3 QMQ = Matrix3::ZERO;
    Matrix3 NMat = Matrix3::ZERO; // Holds a vector for a N*N' that yields a 3x3 matrix
    Matrix3 TMat = Matrix3::ZERO; // Holds a vector for a T*T' that yields a 3x3 matrix
    Matrix3 WMat = Matrix3::ZERO; // Holds a vector for a W*W' that yields a 3x3 matrix
    Array< float > weight;    // weight of k and T
    Array< float > k;   //normal curvature in direction vj
    Array< Vector3 > T; //normalized projection of vj-vi onto tangent plane
    ...
    for(int i = 0; i < geometry.vertexArray.size(); ++i) {  // For each vertex
        ...
        EminusN = (Vector3(1, 0, 0) - geometry.normalArray[i]).length();
        EplusN  = (Vector3(1, 0, 0) + geometry.normalArray[i]).length();
        if(EminusN > EplusN)
            W = (Vector3(1, 0, 0) - geometry.normalArray[i])/EminusN; 
        else
            W = (Vector3(1, 0, 0) + geometry.normalArray[i])/EplusN; 
        WMat = Matrix3::ZERO;
        WMat.setColumn(0, W);

        // Calc Householder matrix Q
        Q = Matrix3::IDENTITY - 2 * (WMat*WMat.transpose());
