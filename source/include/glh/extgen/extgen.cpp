...
int ParseFile(FILE *fpInput, FILE *fpOutput, int header, int footer)
{
	...
	if (!header) {
		...
		fprintf(fpOutput, "#ifndef _WIN32\n");
		fprintf(fpOutput, "    }else if (0 == strcmp (extension, \"GL_VERSION_1_2\") || 0 == strcmp (extension, \"GL_VERSION_1_3\")) {\n");
...
A:
	...
	if (0 == strcmp(token, "GL_VERSION_1_2") || 0 == strcmp(token, "GL_VERSION_1_3")) {
		opengl_core_names = 1;
	...
	if (!header) {
		...
		if (opengl_core_names==1) {
			...
			fprintf(fpOutput, "       GLH_CORE_PREFIX to avoid colliding with the OpenGL 1.2 namespace. */\n");
		}
	}
	goto B;
B:
