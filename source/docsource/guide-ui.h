...
\endhtmlonly
    ...
    %G3D provides a series of class that aid in the implementation of user
...
@section gui G3D::UserInput
    ...
    The G3D::UserInput class consolidates the joystick, keyboard and mouse 
...
allows any polling system to be used as long as its input is properly converted.  
    This class provides a basic set of functions for handling common 3D input events like 
...
<PRE>
    ...
    As a default, the UIFUNCTION events UP, LEFT, DOWN and RIGHT are 
...
for further explanations.
    It is important to note one construct of the example code.  All event 
...
@section fp-camera G3D::FPCameraController
    ...
    The G3D::FPCameraController replaces the deprecated ManualCameraController to 
...
and view vector.
    The FPCameraController is most easily used inside the doSimulation function 
...
vector to reflect the user's input.  
    The following code demonstrates how a child class of GApplet can create a 
...
@section gwindow G3D::GWindow
    ...
    G3D::GWindow is G3D's way of dealing with windows.  <B><I>Many parts of %G3D 
...
G3D::UserInput.
    If a custom window set is not desired, several GWindow child classes 
