...
\endhtmlonly
 ...
 in the \link guidetutorial tutorial \endlink, is:
  ...
  The index array is a G3D::Array<int> or G3D::Array<int16> where every three
  indices into the 3D points in vertexArray creates a triangle (you can of course
  use POINTS, LINES, LINE_STRIP, TRIANGLE_STRIP, QUADS, and QUAD_STRIP with 
  G3D::RenderDevice::sendIndices as well).  If you want to visit each vertex
  in the order it appears in vertexArray, use G3D::RenderDevice::sendSequentialIndices 
  instead.  Just as with sendVertex-style rendering, it is possible to specify 
  normals, colors, and texture coordinates with G3D::RenderDevice::setNormalArray and
  other variations.
