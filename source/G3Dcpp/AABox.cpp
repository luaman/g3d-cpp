...
bool AABox::culledByFinite(
    const Array<Plane>&		plane,
	int&				    cullingPlaneIndex,
	const uint32			inMask,
	uint32&					outMask) const {

	return culledByFinite(plane.getCArray(), plane.size(), cullingPlaneIndex, inMask, outMask);
}
...
bool AABox::culledByFinite(
	...
	return culledByFinite(plane.getCArray(), plane.size(), cullingPlaneIndex, inMask);
}
