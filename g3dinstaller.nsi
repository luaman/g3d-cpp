...
;;
;; Add the include/library G3D directories to the MSVCE8 paths if they don't already exist
;;
Section "Add G3D Directories to MSVCE8"
  ...
  ; put Express 8.0 location into $R0
...
Finish_adding_lib:
  FileWrite $1 "Library Dirs="
  StrCpy $R2 $R1 1024 13
  FileWrite $1 "$INSTDIR\win32-vc8-lib;"
  FileWrite $1 $R2
  Goto Read_vccomponents_loop
...
;;
;; Setup Visual Studio Express 8 syntax highlighter and autoexp.dat
;;
Section "Show G3D Classes in MSVCE8 Debugger"
...
;; Setup autoexp.dat for G3D types for debugging
  ...
  ; Search for MSVCE8
  ReadRegStr $R0 HKEY_LOCAL_MACHINE "SOFTWARE\Microsoft\VCExpress\8.0\Setup\VS" "ProductDir"
  IfErrors No_autoexp

Modify_autoexp:
  FileOpen $0 "$R0\Common7\Packages\Debugger\autoexp.dat" a
  IfErrors No_autoexp
Read_autoexp_loop:    
  FileRead $0 $R1
  IfErrors Finished_autoexp_read
...
No_autoexp:
  MessageBox MB_OK "There was no MSVCE8 autoexp.dat file found!"
